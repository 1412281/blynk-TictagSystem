...
SoftwareSerial reader(RX, TX);         // RX, TX

void           (*resetFunc)(void) = 0; //declare reset function at address 0

byte           tokenHasSet = 0;

// char TOKEN_DEFAULT[] = "582d0b89d56545be9ceaa9f7023a7ce";
...
void loop()
{
    ...
    {
        Serial.println("Has new request");
        uid[i] = reader.read();
        Serial.println(uid[i]);
        i++;
    }
    if (uid[0] != 0) {
        Blynk.virtualWrite(V0, uid[5],uid[6],uid[7],uid[8],uid[9],uid[10],uid[11],uid[12],uid[13],uid[14]);
    }
